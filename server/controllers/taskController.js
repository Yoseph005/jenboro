...
const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const users = await User.find({}).select("name role isActive createdAt");
  ...
  res.json({
    ...
    users,
    last10Task: await Task.find({}).sort({ createdAt: -1 }).limit(10),
  });
});
...
export {
  ...
  trashTask,
  deleteRestoreTask,
  getDashboardStats, // Ensure this is exported
};
